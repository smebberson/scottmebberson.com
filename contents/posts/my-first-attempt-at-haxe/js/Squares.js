...
ApplicationMain.preloader_onComplete = function(event) {
	...
	ApplicationMain.preloader = null;
	if(Reflect.field(Squares,"main") == null) jeash.Lib.jeashGetCurrent().addChild(new Squares()); else Reflect.field(Squares,"main").apply(Squares,[]);
}
...
var Squares = $hxClasses["Squares"] = function() {
	jeash.display.Sprite.call(this);
	this.construct();
};
Squares.__name__ = ["Squares"];
Squares.main = function() {
	jeash.Lib.jeashGetCurrent().addChild(new Squares());
}
Squares.__super__ = jeash.display.Sprite;
Squares.prototype = $extend(jeash.display.Sprite.prototype,{
	construct: function() {
		...
		jeash.Lib.jeashGetCurrent().GetStage().addEventListener(jeash.events.MouseEvent.CLICK,this.handleMouseClick.$bind(this));
		this.drawRectangles();
	}
	...
	,drawRectangle: function() {
		var rectangle = new jeash.display.Shape();
		rectangle.jeashGetGraphics().beginFill(5553919,0.6);
		rectangle.jeashGetGraphics().drawRect(0,0,Squares.rectangleWidth,Squares.rectangleHeight);
		rectangle.jeashGetGraphics().endFill();
		jeash.Lib.jeashGetCurrent().addChild(rectangle);
		var x = Math.floor(Math.random() * jeash.Lib.jeashGetCurrent().GetStage().jeashGetStageWidth() - Squares.rectangleWidth);
		var y = Math.floor(Math.random() * jeash.Lib.jeashGetCurrent().GetStage().jeashGetStageHeight() - Squares.rectangleHeight);
		rectangle.jeashSetX(x > 0?x:0);
		rectangle.jeashSetY(y > 0?y:0);
		return rectangle;
	}
	,handleMouseClick: function(event) {
		while(jeash.Lib.jeashGetCurrent().jeashGetNumChildren() > 0) jeash.Lib.jeashGetCurrent().removeChildAt(0);
		this.drawRectangles();
	}
	,__class__: Squares
});
...
Squares.rectangleWidth = 50;
Squares.rectangleHeight = 50;
